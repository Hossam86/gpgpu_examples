...
__global__ void gaussian_blur(const unsigned char *const inputChannel,
  ...
  // but
  // some of its neighbors are outside the image, then you will need to be
  // extra
  ...
  for (int fr = 0; fr < filterWidth; ++fr) {
    for (int fc = 0; fc < filterWidth; ++fc) {
      int fidx = fc * filterWidth + fr;
      float fv = filter[fidx];
      int dx = fr - filterWidth / 2;
      int dy = fc - filterWidth / 2;
      int img_r = min(max(x + dx, 0), numCols - 1);
      int img_c = min(max(y + dy, 0), numRows - 1);
      float img_v = static_cast<float>(inputChannel[img_c * numCols + img_r]);
      final_value += img_v * fv;
    }
  }
  outputChannel[idx] = final_value;
}
...
__global__ void separateChannels(const uchar4 *const inputImageRGBA,
  ...
  int rgb_idx = y * numCols + x;
  redChannel[rgb_idx] = inputImageRGBA[rgb_idx].x;
  greenChannel[rgb_idx] = inputImageRGBA[rgb_idx].y;
  blueChannel[rgb_idx] = inputImageRGBA[rgb_idx].z;
...
void your_gaussian_blur(const uchar4 *const h_inputImageRGBA,
  ...
  unsigned int THREAD_PER_BLOCK_X = 32;
  unsigned int THREAD_PER_BLOCK_Y = 32;

  const dim3 blockSize(THREAD_PER_BLOCK_X, THREAD_PER_BLOCK_Y, 1);

  // TODO:
  // Compute correct grid size (i.e., number of blocks per kernel launch)
  // from the image size and and block size.
  unsigned int GRIDSIZE_X =
      (numCols + THREAD_PER_BLOCK_X - 1) / THREAD_PER_BLOCK_Y;
  unsigned int GRIDSIZE_y =
      (numRows + THREAD_PER_BLOCK_Y - 1) / THREAD_PER_BLOCK_Y;
  const dim3 gridSize(GRIDSIZE_X, GRIDSIZE_y, 1);

  // TODO: Launch a kernel for separating the RGBA image into different
  // color channels
  ...
  // immediately after launching your kernel to make sure that you didn't
  // make any mistakes.
  cudaDeviceSynchronize();
  checkCudaErrors(cudaGetLastError());
